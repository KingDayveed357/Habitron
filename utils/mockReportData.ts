import { HabitDataProps, AIInsight, PeriodOption, TimePeriod, PeriodData } from '@/interfaces/interfaces';

// Generate mock daily data for the last 2 years
export const generateDailyData = (habitId: string) => {
  const data = [];
  ...
  console.log(`[generateDailyData] Habit ID: ${habitId}, Total Days Generated: ${data.length}`);
  return data;
};
...
// Mock habit data
export const HabitData: HabitDataProps[] = [
  {
    id: '1',
...
export const generateAIInsights = (periodData: PeriodData): AIInsight[] => {
  return [
    {
      type: 'prediction',
...
export const calculatePeriodData = (
  ...
  const overallCompletionRate = filteredData.length > 0
    ? Math.round(filteredData.reduce((sum, habit) => sum + habit.completionRate, 0) / filteredData.length)
    : 0;
  ...
  const getPeriodLabel = (period: TimePeriod, start: Date, end: Date): string => {
    ...
    switch (period) {
      ...
      case 'custom': return `${start.toLocaleDateString('en-US', options)} - ${end.toLocaleDateString('en-US', options)}`;
      default: return 'Unknown Period';
    }
  };

  const label = getPeriodLabel(selectedPeriod, startDate, endDate);
